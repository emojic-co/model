MODEL_DIR = "model"
TOOLS_DIR = "tools"
DATA_DIR = "data"
PT_DIR = "pt"
GOAL_DIR = "goal"

GOALS_YML = "goals.yml"

DATA_JSONL = f"{DATA_DIR}/data.jsonl"
TRAIN_JSONL = f"{DATA_DIR}/train.jsonl"
EVAL_JSONL = f"{DATA_DIR}/eval.jsonl"
LABELS_JSON = f"{DATA_DIR}/labels.json"
II_JSON = f"{DATA_DIR}/ii.json"
CLDR_JSONL = f"{DATA_DIR}/cldr.jsonl"
EMOJILIB_JSONL = f"{DATA_DIR}/emojilib.jsonl"
WA_KEYWORDS_JSON = f"{DATA_DIR}/wa-keywords.json"
KEYWORDS_JSONL = f"{DATA_DIR}/keywords.jsonl"
TERMS_JSONL = f"{DATA_DIR}/terms.jsonl"
FLAGS_JSONL = f"{DATA_DIR}/flags.jsonl"
CLDR_BASELINE_JSON = f"{DATA_DIR}/cldr-baseline.json"
EMOJI_POPULARITY_JSON = f"{DATA_DIR}/emoji_popularity.json"
GROUP_JSON = f"{DATA_DIR}/group.json"
COLORS_JSONL = f"{DATA_DIR}/colors.jsonl"
PRED_JSONL = f"{DATA_DIR}/pred.jsonl"

ENC_PT = f"{PT_DIR}/enc.pt"
LANG_PT = f"{PT_DIR}/lang.pt"
STYLE_PT = f"{PT_DIR}/style.pt"
EMOJI_PT = f"{PT_DIR}/emoji.pt"
EMOJI_EMBED_PT = f"{PT_DIR}/emoji_embed.pt"
GEN_PT = f"{PT_DIR}/gen.pt"
CRITIC_PT = f"{PT_DIR}/critic.pt"

WEB_PUBLIC_DIR = "web/public"
REPORT_DIR = "report"
RUNS_DIR = "runs"
