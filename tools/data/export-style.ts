...
function patternSvg(feeling: string): { name: string; svg: string; widthRatio: number; heightRatio: number } {
  ...
  const name = PATTERN_NAMES.get(entry.pattern)
  if (!name) throw new Error(`no pattern name registered for ${feeling}`)
...
export function buildStyleFile() {
  ...
  const styles = Object.fromEntries(
    Object.entries(FEELINGS).map(([name, f]) => {
      ...
      const pattern = patternSvg(name)
      patterns[pattern.name] = pattern.svg
      return [
        name,
        {
          cluster: f.cluster,
          ...
          pattern: { name: pattern.name, widthRatio: pattern.widthRatio, heightRatio: pattern.heightRatio },
        },
      ]
    }),
  )
