...
- `data/keywords.json` is derived from `data/labels.json` exactly like `data/train.jsonl`/`data/eval.jsonl` are derived from `data/data.jsonl` — it is gitignored and gets regenerated by re-running this skill, never hand-edited.
