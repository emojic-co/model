...
2. **Close the emoji-palette gap.** ~10% of rows carry an off-palette emoji and are silently dropped at load. Either (a) constrain the generator's free-choice emoji to the current `labels.json` palette, or (b) re-run `bun run tools/data/gen_labels.ts` more often / raise the palette from 300, or (c) add an emoji alias/merge table mirroring `FEELING_ALIASES`.
